...
- Capability discovery must fail closed. If live model/agent discovery is unavailable, hangs, is interrupted, returns incomplete output, or otherwise fails to establish availability, treat the concrete capability as **unverified**. Do not fall back to remembered, guessed, or inferred model names, agent APIs, or delegation mechanisms; describe the role generically or state the limitation instead.
